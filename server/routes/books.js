...
router.post("/", (req, res) => {
  const db = getUserDb(req.user.uid);
  const { title, author, category, pages, year, status, start_date, end_date, rating, is_reread } = req.body;
  if (!title) return res.status(400).json({ error: "Título é obrigatório" });

  const result = db.prepare(`
    INSERT INTO books (title, author, category, pages, year, status, start_date, end_date, rating, is_reread)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(title, author ?? "", category ?? "", pages ?? 0, year ?? 0,
...
router.put("/:id", (req, res) => {
  ...
  db.prepare(`
    UPDATE books SET
      title = ?, author = ?, category = ?, pages = ?, year = ?,
